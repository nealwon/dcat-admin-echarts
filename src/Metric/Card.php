...
class Card extends Widget
{
     ...
     * @var string
     */
    protected $view = 'dcat-admin.echarts::card';
    ...
    {
        return $this->chart ?: ($this->chart = Chart::make(null, [], $this->chartHeight));
    }
...
//        $this->chartOptions['chart']['height'] = $this->chartHeight;
        ...
        // 颜色
        if (empty($this->chartOptions['color'])) {
//            $this->chartOptions['color'] = (array)Admin::color()->get($this->style);
        }
...
JS;
    ...
    public function addSeries(ChartSeries $series) {
        if (!isset($this->chartOptions['series'])) {
            $this->chartOptions['series'] = [];
        }
        $this->chartOptions['series'][] = $series->render();
    }

    public function disableX(bool $disabled=true) {
        $this->chartOption('xAxis.show', $disabled!=true);
    }

    public function disableY(bool $disabled=true) {
        $this->chartOption('yAxis.show', $disabled!=true);
    }
}
